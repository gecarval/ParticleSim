...
int exit_data(t_data *data, int fd) {
	...
	if (data != NULL && data->fsim != NULL) free(data->fsim);
	...
	if (data != NULL && data->map != NULL) free(data->map);
	if (data != NULL && data->original_map != NULL &&
		data->original_map->mapz != NULL)
		free(data->original_map->mapz);
	if (data != NULL && data->original_map != NULL) free(data->original_map);
...
void ft_init_program(t_data *data) {
	...
	if (!data->map) display_error(data, "map malloc error\n");
	data->map->parsed_file = NULL;
	data->map->mapz = NULL;
	data->cam = NULL;
	data->fsim = (t_fluidsim *)malloc(sizeof(t_fluidsim));
	if (!data->fsim) display_error(data, "map malloc error\n");
	data->fsim->map = NULL;
