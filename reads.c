...
void create_mapz(t_data *data) {
	...
	map->mapz =
		(t_pt *)malloc(sizeof(t_pt) * map->max_y * map->max_x * map->max_z);
	if (!map->mapz) display_error(data, "mapi malloc error\n");
	map->space = 150;
	data->dist = 3;
	fake_map(map);
	backup_map(data);
...
void create_fsim(t_data *data) {
	...
	if (!data->fsim->map) display_error(data, "error malloc simmap");
	data->fsim->map[WINY] = NULL;
	i = -1;
	while (++i < WINY) {
		j = -1;
		data->fsim->map[i] = (char *)malloc(sizeof(char) * (WINX + 1));
		if (!data->fsim->map[i]) display_error(data, "error malloc simmap");
		while (++j < WINX) {
			if (i == 0 || i == (WINY - 1))
				data->fsim->map[i][j] = 'z';
...
void fake_map(t_map *map) {
	...
	pt.z = -((float_t)map->max_z / 2);
	k = 0;
	while (k < map->max_z) {
		pt.y = -((float_t)map->max_y / 2);
		j = 0;
		while (j < map->max_y) {
			pt.x = -((float_t)map->max_x / 2);
			i = 0;
			while (i < map->max_x) {
				...
				*cur = pt;
				cur->color = 0xffffff;
				pt.x += 1;
				if (pt.x == 0) pt.x = pt.x + 1;
				i++;
			}
			pt.y += 1;
			if (pt.y == 0) pt.y = pt.y + 1;
			j++;
		}
		pt.z += 1;
		if (pt.z == 0) pt.z = pt.z + 1;
		k++;
	}
}

void set_height_range(t_map *map, t_pt *cur) {
	if (cur->z < map->min_z) map->min_z = cur->z;
	if (cur->z > map->max_z) map->max_z = cur->z;
}
...
void fill_map(t_data *data, t_map *map) {
	...
	pt.z = 0;
	pt.y = (float_t)map->space * map->max_y / 2;
	i = 0;
	while (i < map->max_y) {
		pt.x = (float_t)-map->space * map->max_x / 2;
		j = 0;
		while (j < map->max_x) {
...
void backup_map(t_data *data) {
	...
	data->original_map->mapz =
		malloc(data->map->max_x * data->map->max_y * sizeof(t_pt));
